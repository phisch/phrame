...
use skia_safe::{
    ...
    Surface as SkiaSurface, textlayout::Decoration, Size,
};
use smithay_client_toolkit::{
    ...
    shell::{
        ...
        WaylandSurface, xdg::{XdgShell, window::{WindowHandler, WindowDecorations, Window}},
...
fn main() {
    ...
    let (context, surface) = init_egl(window.wl_surface(), 3000, 1000);
...
fn create_surface(
    ...
    // print surface.width()
    println!("surface width: {}", surface.width().unwrap());
...
delegate_xdg_shell!(SimpleLayer);
delegate_xdg_window!(SimpleLayer);
impl WindowHandler for SimpleLayer {
    fn request_close(&mut self, conn: &Connection, qh: &QueueHandle<Self>, window: &smithay_client_toolkit::shell::xdg::window::Window) {
    ...
    fn configure(
        ...
        self.surface.resize(&self.context, NonZeroU32::new(self.width).unwrap(), NonZeroU32::new(self.height).unwrap());
        ...
        self.skia_surface = create_surface(
            &self.surface,
            self.context.config().num_samples().into(),
            self.context.config().stencil_size().into(),
            &self.fb_info,
            &mut gr_context,
        );
